...
# ----- Strategy 2: Static 1.4x (QQQ:QLD:BIL = 4:9.67:3, quarterly rebalance) -----
def backtest_static_14x(prices, divs, start_cap=CAPITAL, w=(4.0, 9.67, 3.0)):
    """Static 1.4x leverage strategy with quarterly rebalancing"""
...
# ----- Strategy 3: Eric's Diversified Strategy (annual rebalancing) -----
def backtest_eric_strategy(prices, divs, weights, start_cap=CAPITAL):
    """Eric's diversified strategy with annual rebalancing"""
...
res_14x = perf_stats(val_14x, rf_daily, "Static 1.4x (QQQ + QLD + BIL)")
res_eric = perf_stats(val_eric, rf_daily, "Eric's Strategy (BIL/QQQ/SCHD/QLD/BTC)")
...
plt.plot(val_14x.index, val_14x, label="Static 1.4x", linewidth=2, color='blue')
plt.plot(val_eric.index, val_eric, label="Eric's Strategy", linewidth=2, color='green')
plt.plot(val_qqq.index, val_qqq, label="QQQ Buy & Hold", linewidth=2, color='orange')
plt.title("Four Strategy Comparison - Portfolio Value", fontsize=14, fontweight='bold')
plt.xlabel("Date")
...
plt.subplot(2, 2, 2)
strategies = ['9Sig', 'Static 1.4x', "Eric's", 'QQQ B&H']
...
print(f"{'Static 1.4x':<25} ${res_14x['final']:>12,.0f} {res_14x['cagr']*100:>6.1f}% {res_14x['mdd']*100:>6.1f}% {res_14x['vol']*100:>10.1f}% {res_14x['sharpe']:>6.2f}")
