...
# All tickers needed for all strategies (removed TQQQ since 9Sig is removed)
...
# ----- Strategy 2: Eric's Diversified Strategy V2 (annual rebalancing, with Gold) -----
...
val_static, df_static = backtest_static_1x(prices, divs)
...
res_static = perf_stats(val_static, rf_daily, "Static 1x (QQQ + QLD + BIL)")
res_eric = perf_stats(val_eric, rf_daily, "Eric's Strategy (BIL/QQQ/SCHD/QLD/GLD)")
...
plt.subplot(2, 2, 1)
plt.plot(val_static.index, val_static, label="Static 1x", linewidth=2, color='blue')
plt.plot(val_eric.index, val_eric, label="Eric's Strategy (w/ Gold)", linewidth=2, color='green')
plt.plot(val_qqq.index, val_qqq, label="QQQ Buy & Hold", linewidth=2, color='orange')
plt.title("Three Strategy Comparison - Portfolio Value", fontsize=14, fontweight='bold')
plt.xlabel("Date")
...
plt.subplot(2, 2, 2)
strategies = ['Static 1x', "Eric's", 'QQQ B&H']
cagrs = [res_static['cagr']*100, res_eric['cagr']*100, res_qqq['cagr']*100]
...
plt.subplot(2, 2, 3)
mdds = [res_static['mdd']*100, res_eric['mdd']*100, res_qqq['mdd']*100]
...
plt.subplot(2, 2, 4)
sharpes = [res_static['sharpe'], res_eric['sharpe'], res_qqq['sharpe']]
...
print("-" * 80)
print(f"{'Static 1x':<25} ${res_static['final']:>12,.0f} {res_static['cagr']*100:>6.1f}% {res_static['mdd']*100:>6.1f}% {res_static['vol']*100:>10.1f}% {res_static['sharpe']:>6.2f}")
eric_name = "Eric's Strategy"
